...
function getMaxthonRecordsFromBrowser(paths, browserName) {
  let browserHistory = [],
      h              = [];
  return new Promise((resolve, reject) => {
    if (!paths || paths.length === 0) {
...
function getMaxthonHistory(paths, browserName) {
  return new Promise(function(resolve, reject) {
    getMaxthonRecordsFromBrowser(paths, browserName).then(foundRecords => {
      records = records.concat(foundRecords);
      resolve(records);
    }, error => {
      reject(error);
...
function getWindowsBrowserHistory(driveLetter, user) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    Promise.all(getPaths).then(function(values) {
      let getRecords = [
        getFireFoxHistory(browsers.firefox, FIREFOX),
        getFireFoxHistory(browsers.seamonkey, SEAMONKEY),
        getStandardHistory(browsers.chrome, CHROME),
  ...
  return new Promise(function(resolve, reject) {
    ...
    Promise.all(getPaths).then(function(values) {
      let getRecords = [
        ...
        getFireFoxHistory(browsers.seamonkey, SEAMONKEY),
        getMaxthonHistory(browsers.maxthon, MAXTHON),
